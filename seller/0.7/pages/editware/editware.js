...
Page({
  ...
  backvalue: function (e) {
    if (e.detail.value.name&&e.detail.value.addr&&e.detail.value.wareman&&e.detail.value.phone) {
      ...
      db.collection('wares').doc(this.data.editor._id).set({
        data: e.detail.value
      })
      setTimeout( function() {
         wx.showToast({
...
delete: function () {
  ...
  const db = wx.cloud.database()
  const _ = db.command
  db.collection('wares').doc(this.data.editor._id).remove()
  db.collection('goods').where({}).update({
    data: {
      ware: {[this.data.editor._id]: _.remove()}
    }
  })
  setTimeout( function() {
    wx.showToast({
