// pages/inshelvedetail/inshelvedetail.js
const app = getApp()
Page({
  ...
  getware: function() {
    ...
    wares.get().then(res => {
        this.data.wares = res.data
        for (var i = 0; i < this.data.wares.length; i++) {
          this.data.warearray[i] = this.data.wares[i].name
        }
        this.setData({
          warearray: this.data.warearray
        })
    })
  },

  w_bindPickerChange: function(e) {
    
    this.data.wareIndex = e.detail.value
    this.setData({
      wareIndex: e.detail.value,
      warename: this.data.warearray[this.data.wareIndex]
    })
    var wareid = this.data.wares[this.data.wareIndex]._id
    this.data.ware_id = wareid
    this.data.datas = []
    for (var i = 0; i < this.data.in.length; i++) {
      if (this.data.in[i].ware._id == wareid) {
        if (this.data.shelve_id == '')
          this.data.datas.push(this.data.in[i])
        else {
          if (this.data.in[i].shelve._id == this.data.shelve_id)
            this.data.datas.push(this.data.in[i])
        }
      }
    }
  ...
  bindPickerChange: function(e) {
    ...
    for (var i = 0; i < this.data.in.length; i++) {
      if (this.data.in[i].shelve._id == shelveid) {
        if (this.data.ware_id == '')
          this.data.datas.push(this.data.in[i])
        else {
          if (this.data.in[i].ware._id == this.data.ware_id)
            this.data.datas.push(this.data.in[i])
        }
      }
    }
  ...
  getdata: function() {
    ...
    const db = wx.cloud.database()
    const wares = db.collection('shelveinlog') 
    wares.orderBy('date', 'desc').get().then(res => {
